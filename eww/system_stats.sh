#!/bin/bash
# A more efficient and organized system monitoring script for Eww.
...
get_cpu_usage() {
    ...
    local diff_idle=$((total_idle - last_total_idle))
    local diff_total=$((total_time - last_total_time))
...
get_temperature() {
    # Safe read from direct path using 'head' for better compatibility
    local temp_file="/sys/class/thermal/thermal_zone0/temp"
    if [[ -f "$temp_file" && -r "$temp_file" ]]; then
        local temp_value
        # Using 'head' instead of 'cat' as it can be more reliable in some contexts
        temp_value=$(head -n 1 "$temp_file")
...
get_network() {
    if [[ -d "/sys/class/net/$NETWORK_INTERFACE" ]]; then
        ...
        current_rx=$(cat "/sys/class/net/$NETWORK_INTERFACE/statistics/rx_bytes")
        current_tx=$(cat "/sys/class/net/$NETWORK_INTERFACE/statistics/tx_bytes")
        local time_diff=$((current_time - last_net_time))
        if (( time_diff > 0 )); then
            local rx_speed=$(((current_rx - last_rx_bytes) * 1000000000 / time_diff))
            local tx_speed=$(((current_tx - last_tx_bytes) * 1000000000 / time_diff))
...
last_total_idle=$((last_idle + last_iowait))
last_total_time=$((last_user + last_nice + last_system + last_idle + last_iowait + last_irq + last_softirq + last_steal))
if [[ -d "/sys/class/net/$NETWORK_INTERFACE" ]]; then
    last_net_time=$(date +%s%N)
    last_rx_bytes=$(cat "/sys/class/net/$NETWORK_INTERFACE/statistics/rx_bytes")
    last_tx_bytes=$(cat "/sys/class/net/$NETWORK_INTERFACE/statistics/tx_bytes")
fi
...
while true; do
    ...
    read -r net_down net_up <<< "$(get_network)"
